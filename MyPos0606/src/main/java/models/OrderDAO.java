...
public class OrderDAO {
    ...
    conn = DBConnection.getConnection();
    String query = "INSERT INTO `order_detail` (`order_num`, `product_id`, `quantity`, `condensedmilk`) VALUES (?, ?, ?, ?)";
    boolean success = false;
    try {
        PreparedStatement state = conn.prepareStatement(query);
        ...
        state.setBoolean(4, item.isAddCondensedMilk());
        state.execute();
        success = true;
        System.out.println("Insert order detail 成功!");
    } catch (SQLException ex) {
        System.out.println("Insert 異常: " + ex.toString());
    }
    return success;
}
    ...
    public static void main(String[] args) {
        ...
        //測試1--------------------
        //訂單編號
        String ordNum = "ord-101";

        //新增訂單物件
        Order cart = new Order();
        
        cart.setOrder_num(ordNum);
        cart.setTotal_price(110);
        cart.setCustomer_name("李大同");
        cart.setCustomer_address("高雄市楠梓區大學路一號");
        cart.setCustomer_phtone("093256789");
        orddao.insertCart(cart);

        //新增訂單細節 
        OrderDetail new_item = new OrderDetail();
        
        new_item.setProduct_id("p-i-103");
        ...
        order_list = FXCollections.observableArrayList(
                new OrderDetail("p-t-203", "布丁", 15, 3,false),
                new OrderDetail("p-i-105", "新鮮草莓雪花冰", 75, 1,false)
        );
